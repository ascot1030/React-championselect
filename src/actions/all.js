...
export const REQUEST_MOST_POPULAR_CHAMPIONS = 'REQUEST_MOST_POPULAR_CHAMPIONS';
export const RECEIVE_MOST_POPULAR_CHAMPIONS = 'RECEIVE_MOST_POPULAR_CHAMPIONS';

export function fetchMostPopularChampionsIfNeeded (params) {
  return (dispatch, getState) => {
    if (shouldFetchMostPopularChampions(getState())) return dispatch(fetchMostPopularChampions('/mostpopularchampions'));
  };
}

function shouldFetchMostPopularChampions (state) {
  const {
    all: {
  ...
  if (mostPopularChampions.isFetching)
    return false;

  return true;
}

function fetchMostPopularChampions (url) {
  return dispatch => {
    dispatch(requestMostPopularChampions());
    ...
    return fetch(url)
      ...
      .then(json => dispatch(receiveMostPopularChampions(json)))
      .catch (error => {
...
function requestMostPopularChampions () {
  return {
    type: REQUEST_MOST_POPULAR_CHAMPIONS
  };
}

function receiveMostPopularChampions (data) {
  return {
    type: RECEIVE_MOST_POPULAR_CHAMPIONS,
    data
  };
}
